...
| file | lines | origin |
|---|---|---|
| `hammer2.h` | 1399 | DragonFly, in the FreeBSD port's shape, OS-facing types rewritten |
...
| `hammer2_vfsops.c` | 3275 | FreeBSD port; the PFS half and the recovery carried, the module entry, globals, mount path, mount helper, evict_inode, and sops this port's. A rewrite with a carried body, since Linux redistributes `hammer2_mount()` across four `fs_context` callbacks |
| `hammer2_strategy.c` | 1426 | this port's; `hammer2_dedup_clear()` carried, both XOP handlers are floors |
| `hammer2_vnops.c` | 1447 | this port's; `->lookup` is upstream's `hammer2_lookup()` with the dcache's own cases and the nameiop pre-checks dropped, and the four operations tables have no BSD counterpart, a vnode taking its vop vector from the mount rather than from its type |
