...
| `hammer2_vfsops.c` | 664 | FreeBSD port; the PFS half carried, the module entry and the globals this port's, the mount path itself not written yet. A rewrite with a carried body, since Linux redistributes `hammer2_mount()` across four `fs_context` callbacks |
...
Sixty-seven are this port's, the right-hand column summed. Nineteen
...
against `iget5_locked()`, five with `hammer2_vfsops.c` and three more when
that file gained the module entry. Fifty-nine sit in
