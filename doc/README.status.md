...
| `hammer2_vfsops.c` | 678 | FreeBSD port; the PFS half carried, the module entry and the globals this port's, the mount path itself not written yet. A rewrite with a carried body, since Linux redistributes `hammer2_mount()` across four `fs_context` callbacks |
