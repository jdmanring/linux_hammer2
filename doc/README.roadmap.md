...
| 0.1.48 | 2026-08-26 | a document can state the right COUNT and enumerate the wrong LIST, and those are different claims about one population: `CLAUDE.md` said eight gates and listed seven, so the count check passed while the list a future reader runs was short by the newest gate - in the first file an agent working here reads (`55d5860`) | the inventory gate requires every gate to be named in the three documents printing runnable command lists; documents that merely mention one are deliberately excluded, since requiring all eight there would be wrong. Falsified by removing a name from `README.md` |
