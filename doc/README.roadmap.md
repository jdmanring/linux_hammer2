...
| 0.1.8 | 2026-08-25 | the 0.1.x rows audited claim by claim against the tree rather than against this table, and the one overclaim found corrected: `README.status.md` said "the first eight of those are expected to carry", which put `inode` and `subr` in the carried set on no cited authority and pre-decided what 0.2 routes to the provenance CSV. The measured set is the six of `H1_ESTIMATE.md`. A gate now reads every `file:line` citation in `doc/` against the line it names | `script/test-citations.sh`, falsified three ways one invocation at a time: a one-line drift, a citation to a file that does not exist, and an emptied population, which returns COULD-NOT-RUN rather than passing |
