...
| 0.1.46 | 2026-08-26 | the absent-`perl` path was recorded UNVERIFIED because emptying `PATH` breaks the shell and perl shares a directory with everything the gate needs. Both true, conclusion wrong: a nix machine keeps each tool in its own store path, so a `PATH` without perl is ASSEMBLED rather than subtracted (`88f8003`) | built from store coreutils, sed, grep, diff, awk and bash, the gate exits 2 naming `no perl`. "I cannot check this" is itself a claim about an instrument and decays like any other; the honest shape is UNVERIFIED BY THIS ROUTE with the route named, since "perl cannot be hidden" carries no trace of "by directory, on a PATH-shaped machine". Same error as the ArtNix session's "no POSIX shell here" from `command -v`, and it survived the same way: a concession reads as rigour, so nobody audits it |
