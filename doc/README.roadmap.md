...
| 1.0 | qualification | Flagship qualification | open. The bar's items are readings this document records above, each on its named instrument. The two that hold it are outward-facing, so they are the maintainer's rather than the tree's: a tag, and the staged filings filed |
