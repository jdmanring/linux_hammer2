...
| 0.1.38 | 2026-08-26 | the style gate's default could not find the kernel of record's own checker either, the same defect one file over: it looked only at the host's build tree while a `checkpatch.pl` sat in the store under `source/scripts` (`358f749`) | three tiers, strongest first - the file the baseline was produced with by `sha256`, then any checker whose own tree reports the baseline's version, then whatever exists so a refusal can name it. ALL SEVEN GATES NOW RUN GREEN WITH NO ENVIRONMENT VARIABLES SET, against 7.2.0-cachyos, and ArtNix's delegator reports 7 ran / 0 could not run where it read 5 and 2 an hour earlier |
