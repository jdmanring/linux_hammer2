...
0.7.47 row of `CHANGELOG.md` pins, unless a row says otherwise. The
...
| WritableSnapshot | limited | a HAMMER2 snapshot is a PFS root and mounts read-write by format, which DragonFly does; here it has been mounted read-only only | a write into a mounted snapshot read back on both sides |
| Clone | limited | the same object as WritableSnapshot: the snapshot is a full clone of the PFS root, sharing blocks by copy on write | the same measurement |
...
| MultiDevice | limited | the format spans up to 64 volumes and the port parses a colon-separated device list; one volume has been measured, `volume-list` reporting one | a mount over two volumes, written here and read on DragonFly |
